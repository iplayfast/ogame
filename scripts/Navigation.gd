# Navigation.gd - Handles village navigation regions
...
func _ready():
	...
	# Wait a few frames to ensure all buildings are added before creating the navigation
	...
	print("Navigation system initialized properly")
...
func create_navigation_polygon():
	...
	print("Navigation polygon created with " + str(parent.buildings.size() if parent and "buildings" in parent else 0) + " obstacles")
...
func add_building_obstacle(nav_polygon, building):
	...
	# Create obstacle outline with buffer space around the building
	var half_size = size / 2 + Vector2(5, 5)  # Add 5 pixel buffer
